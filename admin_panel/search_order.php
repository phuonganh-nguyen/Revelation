...
<div class="main-container">
        ...
        <section class="order-container">
            <form action="search_order.php" method="post" class="search-form"> 
                    <input type="text" name="search_order" placeholder="Tìm kiếm đơn hàng" required maxlength="100">
                    <button type="submit" class="fas fa-search" id="search_product_btn"></button>
            </form>
            <div class="heading" style="margin-top: 2rem;">
                <h1>Kết quả tìm kiếm cho từ khoá "<?= $_POST['search_order']?>" </h1>
            </div>
            <div class="box-container">
                <?php 
                    if (isset($_POST['search_order']) or isset($_POST['search_product_btn'])) {
                        $search_order = $_POST['search_order'];
                        $select_order = $conn->prepare("SELECT * FROM `hoadon` WHERE (hoadon_id LIKE '%{$search_order}%' 
                            or name LIKE '%{$search_order}%' or user_id LIKE '%{$search_order}%')");
                        $select_order->execute();

                        // echo '<p> ' . $search_products . '</p>';
                        if ($select_order->rowCount() > 0) {
                            while ($fetch_order = $select_order->fetch(PDO::FETCH_ASSOC)) {
                                $hoadon_id = $fetch_order['hoadon_id'];
                ?>
                <div class="box">
