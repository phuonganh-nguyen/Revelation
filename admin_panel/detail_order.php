            ...
            <div class="box-container">
                <div class="order-flex-container">
                    ...
                    <div class="order-right">
                        <div class="order-box">
                        <?php
                            // Kiểm tra nếu có ngày xác nhận
                            if ($fetch_bill['ngaydonghang'] !== null) {
                                // Lấy tên nhân viên duyệt đơn
                                $admin_id = $fetch_bill['admin_id'];  // Lấy admin_id từ bill
                                $select_admin = $conn->prepare("SELECT name FROM `user` WHERE user_id = ?");
                                $select_admin->execute([$admin_id]);
                                $fetch_admin = $select_admin->fetch(PDO::FETCH_ASSOC);
                                
                                if ($fetch_admin) {
                                    echo "<p><strong>Nhân viên duyệt đơn: " . $fetch_admin['name'] . "</strong></p>";
                                }
                            } if ($fetch_bill['ngayhuy'] !== null) {
                                // Lấy tên nhân viên duyệt đơn
                                $admin_id = $fetch_bill['admin_id'];  // Lấy admin_id từ bill
                                $select_admin = $conn->prepare("SELECT name FROM `user` WHERE user_id = ?");
                                $select_admin->execute([$admin_id]);
                                ...
                                if ($fetch_admin) {
                                    echo "<p><strong>Nhân viên hủy đơn: " . $fetch_admin['name'] . "</strong></p>";
                                }
                            }
                        ?>
