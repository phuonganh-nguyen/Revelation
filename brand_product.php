...
<body>
    ...
    <div class="products" >
        <?php 
            $select_thuonghieu = $conn->prepare("SELECT ten_thuonghieu FROM thuonghieu WHERE thuonghieu_id = ?");
            $select_thuonghieu->execute([$brand_id]);
            $thuonghieu_name = $select_thuonghieu->fetch(PDO::FETCH_ASSOC)['ten_thuonghieu'];
        ?>
        <div class="heading">
            <h1 style="text-transform: uppercase;"><?= $thuonghieu_name; ?></h1>
        </div>
        <div class="box-container" style="margin-top: -1rem;">
            <?php 
                if (isset($_GET['brand_id'])) {
                    $select_brand = $conn->prepare("SELECT ten_thuonghieu FROM thuonghieu WHERE thuonghieu_id = ?");
                    $select_brand->execute([$brand_id]);
                    $brand_name = $select_brand->fetch(PDO::FETCH_ASSOC)['ten_thuonghieu'];

                    $select_products = $conn->prepare("SELECT * FROM `sanpham` WHERE BINARY thuonghieu = ? AND trangthai=?");
                    $select_products->execute([$brand_name, 'Đang hoạt động']);
