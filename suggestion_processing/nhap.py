...
try:
    ...
    for product in products:
        ...
        if combined_attributes_str:
            ...
            recommended_products.append((user_id, product['sanpham_id'], similarity, product))
    ...
    # In ra log thông tin chi tiết của 12 sản phẩm
    log("12 sản phẩm có độ tương đồng cao nhất:")
    for rank, (user_id, sanpham_id, similarity, product) in enumerate(recommended_products, start=1):
        log(f"{rank}. Product ID: {sanpham_id} | Similarity: {similarity:.4f}")
        log(f"   Attributes: {product}")

        # In ra terminal (tùy chọn)
        print(f"{rank}. Product ID: {sanpham_id} | Similarity: {similarity:.4f}")
        print(f"   Attributes: {product}")
    ...
    cursor.executemany(""" 
        ...
        VALUES (%s, %s, %s) 
    """, [(rec[0], rec[1], rec[2]) for rec in recommended_products])
    conn.commit()
